 ...
 */
export function socketPath(sessionDir, name) {
    if (IS_WINDOWS) {
 ...
 */
export function metadataPath(sessionDir, name) {
...
export function resolveCommand(command) {
    if (!IS_WINDOWS)
        return { shell: command[0], args: command.slice(1) };
    const cmd = command[0];
